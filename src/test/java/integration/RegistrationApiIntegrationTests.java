...
import com.sadtask.sadtaskApplication;
...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.MOCK, classes = sadtaskApplication.class)
@ActiveProfiles("test")
...
public class RegistrationApiIntegrationTests {
  ...
  public void register_validPayload_shouldSucceedAndReturn201() throws Exception {
    RegistrationPayload payload = payload("sunny", "sunny@sadtask.com");
    mvcMock.perform(
  ...
  public void register_existedUsername_shouldFailAndReturn400() throws Exception {
    RegistrationPayload payload = payload("exist", "test1@sadtask.com");
    mvcMock.perform(
      post("/api/registrations")
        .contentType(MediaType.APPLICATION_JSON)
        .content(JsonUtils.toJson(payload)))
    ...
    RegistrationPayload payload2 = payload("exist", "test2@sadtask.com");
    mvcMock.perform(
      post("/api/registrations")
        .contentType(MediaType.APPLICATION_JSON)
  ...
  public void register_existedEmailAddress_shouldFailAndReturn400() throws Exception {
    RegistrationPayload payload = payload("test1", "exist@sadtask.com");
    mvcMock.perform(
      post("/api/registrations")
        .contentType(MediaType.APPLICATION_JSON)
        .content(JsonUtils.toJson(payload)))
    ...
    RegistrationPayload payload2 = payload("test2", "exist@sadtask.com");
    mvcMock.perform(
      post("/api/registrations")
        .contentType(MediaType.APPLICATION_JSON)
        .content(JsonUtils.toJson(payload2)))
